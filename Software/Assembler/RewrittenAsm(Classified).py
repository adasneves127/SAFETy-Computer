...
from assemblyDef import macros, opCodes, reservedLabels
...
class Instruction:
    ...
    def parseMacro(self, Line: List[str]):
        for i in range(len(line)):
            if line[i] in macros:
                self.parseMacro(macros[line[i]])
            else:
                self.assembly.append(line[i])


    def parseLine(self):
        if self.instruction in macros:
            self.Instructions.append(self.parseMacro(macros[self.instruction]))
    ...
    def __init__(self, Line: List[str]):
        self.instruction: str = Line[0]
        self.operands: List[str] = Line[1:]
        self.parseLine();
